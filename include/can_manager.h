...
//   - Owns optional service lifecycle (logger / USB forward) when enabled.
//
// The application never calls anything in this file after init.
// It simply reads from the global state structs in can_state.h.
// =============================================================================

// Start the manager task and optional services. Must be called after can_driver_init().
esp_err_t can_manager_init(void);

// Stop the manager task and owned optional services. Call before can_driver_deinit().
